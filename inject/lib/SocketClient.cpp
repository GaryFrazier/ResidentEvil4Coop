...
int bytes_for_package_size = 16;
int size_of_packages = 2048;
...
void SocketClient::initSocket(std::string ip, int port)
{
    ...
    this->socket = _WINSOCKAPI_::socket(AF_INET, SOCK_STREAM, 0);
...
{
    int r = _WINSOCKAPI_::connect(socket, (SOCKADDR *)&addr, sizeof(addr));
    if(r==SOCKET_ERROR)
    {
...
void SocketClient::close()
{
    ...
    removeErrorCallback();
    _WINSOCKAPI_::closesocket(socket);
    WSACleanup();
}
...
void SocketClient::send(std::istream &stream)
{
    ...
    result = _WINSOCKAPI_::send(socket, strSize.c_str(), bytes_for_package_size, 0);
    ...
    for (unsigned int i=0 ; i<streamsize/size_of_packages ; i++)
    {
        ...
        result = _WINSOCKAPI_::send(socket, buffer, size_of_packages, 0);
        if(errorSending(result))
            return;
    ...
    if(p!=0)
    {
        ...
        stream.read(buff, p);
        result = _WINSOCKAPI_::send(socket, buff, p, 0);
        if(errorSending(result))
            return;
    }
}
...
std::string SocketClient::receive()
{
    ...
    int result = _WINSOCKAPI_::recv(socket, buffer, bytes_for_package_size, 0);
    ...
    for (unsigned int i=0 ; i<nbLoop ; i++)
    {
        ...
        result = _WINSOCKAPI_::recv(socket, buff, size_of_packages, 0);
